...
class HGattn(nn.Module):
    ...
    def __init__(self,dim_in,dim,pe,timesteps):
        ...
        self.coeff=0.4
    def forward(self,x,joint_label,he_weight,og_joint,og_he_weight):
        op=self.hgc1(x,joint_label,he_weight,og_joint,og_he_weight)
...
class FAHT(nn.Module):
    ...
    def forward(self, x,joint_label,groups,he_weight,og_joint,og_he_weight):
        x = self.l1(x, joint_label, groups,he_weight,og_joint,og_he_weight)
        x = self.l2(x, joint_label, groups,he_weight,og_joint,og_he_weight)
        return x
###########-------Frame_Attentive_Hypergraph_Transformer(FAHT)-------###########
...
##################-----------Weight finder in the hypergraph quantizer -----------##################
...
##################-----------In phase Quantized Hypergraph Generator -----------##################   
class hyp_gen(nn.Module):
    def __init__(self,num_features,out_channels,n_points):
        super(hyp_gen,self).__init__()
        self.dec=Hypergraph_Decoder(in_channels=num_features,out_channels=out_channels) 
        #self.fab=FAB(n_points)
        self.weight_find = WeightMlp(num_points, squeeze_factor=5) 
    ...
    def forward(self,x,edge_index):
        ...
        qe,emb_quant,_,_,ind=self.vq(emb1)
        #weight
        attn=self.weight_find(emb_quant) # [128,1,1,25]
...
class Hypergraph_Encoder(nn.Module):
    ...
    def forward(self, x,joint_label,he_weight):
        ...
        x = self.faht1(x, joint_label, groups,he_weight,joint_label,he_weight)
        x = self.faht2(x, joint_label, groups,he_weight,joint_label,he_weight)
        he_weight1,jl1,emb,rec,qe=self.gen(x,edge_index,joint_label,he_weight) # In phase quantized hypergraph generator
        x = self.faht3(x, jl1, groups,he_weight1,joint_label,he_weight)
        x = self.faht4(x, jl1, groups,he_weight1,joint_label,he_weight)

        x = self.faht5(x, jl1, groups,he_weight1,joint_label,he_weight)
...
class Autoreg_Ad_HGformer(nn.Module):
    ...
    def __init__(self,num_class,num_features,out_channels,n_points,n_person, graph=None, graph_args=dict(),k=5,drop_out=0):
        ...
        if graph is None:
            ...
            Graph = import_class(graph)
            self.graph = Graph(**graph_args)
        self.classifier_model = Hypergraph_Encoder(nf=num_features,oc=out_channels,num_class=num_class,num_person=n_person,num_point=n_points,in_channels=out_channels,graph=graph,graph_args=graph_args)
    ...
    ##############--------Model Agnostic Attentive_Hypergraph__Generator--------##############
    ...
    def forward(self,x,y,tr=True,jl=None,he=None):
        ...
        ####----Classifier----####
        emb ,rec,qe = self.classifier_model(x,joint_label=jl,he_weight=he)  #[128, 216, 16, 25]
        ...
        op = self.drop_out(op)
        op = self.fc(op)
        return op, inp, recon,rec,qe, joint_label, he_weight
##################-----------Autoreg_Ad_HGformer-----------##################
